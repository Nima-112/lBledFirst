    ...
    <MeShell title={t("me.formations.title")}>
      {mine.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-16 text-center">
          ...
          <Link to="/formations" className="mt-4 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-warm transition hover:scale-[1.02]">
          ...
          >
            ...
            <div className="flex flex-1 flex-col p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-1.5 text-[10px] font-semibold text-muted-foreground">
                <span className="inline-flex items-center gap-1 rounded-md bg-muted/60 px-2 py-1"><Clock className="h-3 w-3" /> {formatDuration(f.totalDuration || 0)}</span>
                <span className="inline-flex items-center gap-1 rounded-md bg-muted/60 px-2 py-1"><PlayCircle className="h-3 w-3" /> {totalCapsules(f)}</span>
                <span className="inline-flex items-center gap-1 rounded-md bg-muted/60 px-2 py-1"><GraduationCap className="h-3 w-3" /> {f.level}</span>
              </div>
            </div>
          </Link>
        ))}
