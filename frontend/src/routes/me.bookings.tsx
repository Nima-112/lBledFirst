...
function MyBookings() {
  ...
  const mine = useMemo(
    () => bookings.filter((b) => b.touristId === user?.id).sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
    [bookings, user?.id],
  );

  const reviewed = new Set(reviews.filter((r) => r.touristId === user?.id).map((r) => r.experienceId));
  ...
  return (
    <MeShell title={t("me.bookings.title")}>
          ...
          return (
            <div key={b.id} className="grid gap-3 border-b border-border px-5 py-4 last:border-0 sm:grid-cols-[1.4fr_1fr_auto] sm:items-center">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{ex?.region} · {b.date} · {b.guests} {t("me.bookings.guests")}</p>
              </div>
      ...
      <Modal open={!!reviewFor} title={t("me.bookings.reviewTitle")} onClose={() => setReviewFor(null)} onSave={submitReview}>
        <p className="text-sm text-muted-foreground">{reviewFor ? experienceById(reviewFor.experienceId)?.title : ""}</p>
